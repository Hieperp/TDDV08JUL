...
namespace TotalDAL.Helpers.SqlProgrammability.Productions
{
    public class ProductionOrder
    {
        ...
        private void GetProductionOrderIndexes()
        {
            ...
            queryString = queryString + "       SELECT      ProductionOrders.ProductionOrderID, CAST(ProductionOrders.EntryDate AS DATE) AS EntryDate, ProductionOrders.Reference, ProductionOrders.Code, Locations.Code AS LocationCode, ISNULL(VoidTypes.Name, CASE ProductionOrders.InActivePartial WHEN 1 THEN N'Hủy một phần đh' ELSE N'' END) AS VoidTypeName, ProductionOrders.Description, ProductionOrders.TotalQuantity, ProductionOrders.TotalQuantitySemifinished, ProductionOrders.Approved, ProductionOrders.InActive, ProductionOrders.InActivePartial " + "\r\n";
        ...
        private void GetProductionOrderViewDetails()
        {
            ...
            queryString = queryString + "                   ProductionOrderDetails.Quantity, ProductionOrderDetails.InActivePartial, ProductionOrderDetails.InActivePartialDate, ProductionOrderDetails.Remarks " + "\r\n";
